...
<script type="text/javascript">
	...
	$(function(){
				...
				$.ajax({
					...
					success:function(result){
						$.each(result.uList,function(i,n){
							if (n.name == result.clue.owner){
								html += "<option value='"+n.id+"' selected>"+n.name+"</option>";
							}else {
								html += "<option value='"+n.id+"'>"+n.name+"</option>"
							}
						})
						html += $("#edit-clueOwner").html(html);
						html += $("#edit-company").val(result.clue.company);
						html += $("#edit-call").val(result.clue.appellation);
						html += $("#edit-surname").val(result.clue.fullname);
						html += $("#edit-job").val(result.clue.job);
						html += $("#edit-email").val(result.clue.email);
						html += $("#edit-phone").val(result.clue.phone);
						html += $("#edit-website").val(result.clue.website);
						html += $("#edit-mphone").val(result.clue.mphone);
						html += $("#edit-status").val(result.clue.status);
						html += $("#edit-source").val(result.clue.source);
						html += $("#edit-describe").val(result.clue.description);
						html += $("#edit-contactSummary").val(result.clue.contactSummary);
						html += $("#edit-nextContactTime").val(result.clue.nextContactTime);
						html += $("#edit-address").val(result.clue.address);
					}
				})
		...
		//全选操作
		$("#clueEditCheckBox").click(function(){
			$("input[name=xz]").prop("checked",this.checked);
		})
		$("#clueBody").on("click","input[name=xz]",function(){
			$("#clueEditCheckBox").prop("checked",$("input[name=xz]").length==$("input[name=xz]:checked").length);
		});
		...
		$("#delBtn").click(function(){
			...
			if($jq.length == 0){
				alert("至少选择一条进行删除~~~~");
			}else if (window.confirm("你确定要删除这" + $jq.length+"条线索吗？")){
	...
	function pageList(pageNo,pageSize) {
		//将全选框的√去掉
		$("#clueEditCheckBox").checked =false;
		$.ajax({
			...
			success: function (result) {
				...
				$.each(result.dataList, function (i, n) {
					...
					html += '<td><a style="text-decoration: none; cursor: pointer;"' +
							' onclick="window.location.href=\'clue/detail.do?id='+n.id+'\';">' + n.fullname+n.appellation + '</a></td>';
						...
						<div class="form-group">
							...
							<div class="col-sm-10" style="width: 300px;">
								<select class="form-control" id="create-status">
									<c:forEach items="${applicationScope.get('clueState')}" var="b">
		...
		<div style="width: 100%; position: absolute;top: 5px; left: 10px;">
			...
			<div class="btn-toolbar" role="toolbar" style="background-color: #F7F7F7; height: 50px; position: relative;top: 40px;">
				<div class="btn-group" style="position: relative; top: 18%;">
				  ...
				  <button type="button" class="btn btn-danger"><span class="glyphicon glyphicon-minus" id="delBtn"></span> 删除</button>
				</div>
				...
				<table class="table table-hover">
					<thead>
						<tr style="color: #B3B3B3;">
							<td><input type="checkbox" id="clueEditCheckBox"/></td>
							<td>名称</td>
							<td>公司</td>
							<td>公司座机</td>
							<td>手机</td>
